...
next unchecked item, checks it off, and writes what it did into PROJECT.md's "Recent
work" log before ending its turn.** Checkboxes are the source of truth for what's done;
keep them accurate rather than tidy. Read PROJECT.md first for the contract and file map.
...
- [x] `src/lib/types.ts`: widen mode to `'mock' | 'live' | 'stale'` and add
  `warning?: string` to `WidgetResponse`.
  → verify: `npx tsc --noEmit` fails only where the new state must be handled.
- [x] Connector handlers: on a live failure return the mock **plus** the reason, e.g.
...
- [x] `WidgetShell.tsx`: `stale` renders the data plus an amber header pill whose tooltip is
...
- [x] Add `DEBUG_CONNECTORS=1` env: when set, log every outbound request (method, url,
  status, ms) to the server console, never the key.
  → verify: one line per widget fetch, secrets absent from output.

See PROJECT.md "Recent work" for exactly what was built and how it was verified.
...
- [ ] **Caching.** Add `src/lib/cache.ts`: in-memory TTL map keyed by
...
- [ ] **RSS SSRF guard.** The feed URL is user input. Reject non-http(s) schemes, and reject
...
- [ ] **System GPU.** Only path needing the target machine (Ryzen 5 / RTX 3070). Run
...
- [ ] Create a **restricted** key in the Stripe dashboard, read-only on Balance + Charges.
   Put it in `.env.local`. Never a live-mode full secret key.
- [ ] Verify field mapping against a real response for `/v1/balance` and `/v1/charges`:
...
- [ ] `stripe.revenue`: current code sums up to 100 charges. Implement cursor pagination
   ...
   Stripe's own report.
- [ ] Decide refunds/disputes: gross volume should probably subtract refunds. Pick one,
   document it in the widget description.

## Phase 3 — GitHub (~1–2h)

- [ ] Fine-grained PAT, read-only, no repo write scopes. `.env.local`.
- [ ] Verify `/users/<u>/events` mapping for each type handled (PushEvent commit count,
...
- [ ] `github.contributions` uses the GraphQL API — confirm the PAT type actually has
...
- [ ] Add conditional requests: store `ETag` per endpoint, send `If-None-Match`, treat 304 as
...
- [ ] Google Cloud project → OAuth client (Desktop or Web). Scopes:
   `calendar.readonly`, `gmail.readonly`. New env:
   `GOOGLE_CLIENT_ID`, `GOOGLE_CLIENT_SECRET`, `GOOGLE_REDIRECT_URI`.
- [ ] Add routes `src/app/api/auth/google/route.ts` (redirect to consent, `access_type=offline`,
...
- [ ] `src/lib/google.ts`: `getAccessToken()` — reads the refresh token, exchanges it, caches
   the access token in memory until `expires_in - 60s`. Both connectors call only this.
- [ ] Replace `envKeys` for gcal/gmail with a credential check against the stored file so
...
- [ ] Gmail: current code does N+1 requests (list + per-message metadata). Use
...
- [ ] Calendar: confirm `singleEvents=true&orderBy=startTime`, all-day events use `start.date`
...
- [ ] → verify the whole phase: leave the dashboard open for >1h; widgets keep working after
