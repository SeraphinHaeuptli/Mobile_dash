...
const connector: ConnectorServer = {
  ...
  isLive: () => hasEnv(ENV),
  handlers: {
    'gcal.agenda': (s) => withFallback(() => liveAgenda(s), () => mockAgenda(s), { envKeys: ENV, label: 'gcal.agenda' }),
    'gcal.next': (s) => withFallback(() => liveNext(s), () => mockNext(s), { envKeys: ENV, label: 'gcal.next' }),
  },
};
