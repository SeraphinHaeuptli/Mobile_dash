...
const connector: ConnectorServer = {
  ...
  handlers: {
    'github.activity': (s) => {
      ...
      return withFallback(
        hasEnv(ENV),
        () => cached('github.activity', s, TTL_SECONDS, () => liveActivity(username, limit)),
    ...
    'github.repos': (s) => {
      ...
      return withFallback(
        hasEnv(ENV),
        () => cached('github.repos', s, TTL_SECONDS, () => liveRepos(username, sort, limit)),
    ...
    'github.contributions': (s) => {
      ...
      return withFallback(
        hasEnv(ENV),
        () => cached('github.contributions', s, TTL_SECONDS, () => liveContributions(username)),
