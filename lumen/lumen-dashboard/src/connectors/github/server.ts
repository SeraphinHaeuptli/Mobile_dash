...
async function gh(path: string, init?: { method?: string; body?: string }): Promise<unknown> {
  ...
  try {
    const res = await debugFetch('github', `${API}${path}`, {
...
const connector: ConnectorServer = {
  ...
  handlers: {
    'github.activity': (s) => {
      ...
      return withFallback(
        hasEnv(ENV),
        () => cached(cacheKeyFor('github.activity', s), CACHE_TTL_SECONDS, () => liveActivity(username, limit)),
    ...
    'github.repos': (s) => {
      ...
      return withFallback(
        hasEnv(ENV),
        () => cached(cacheKeyFor('github.repos', s), CACHE_TTL_SECONDS, () => liveRepos(username, sort, limit)),
    ...
    'github.contributions': (s) => {
      ...
      return withFallback(
        hasEnv(ENV),
        () => cached(cacheKeyFor('github.contributions', s), CACHE_TTL_SECONDS, () => liveContributions(username)),
