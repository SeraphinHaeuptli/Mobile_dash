...
const connector: ConnectorServer = {
  ...
  handlers: {
    'stripe.balance': (s) =>
      withFallback(hasEnv(ENV), () => liveBalance(s), () => mockBalance(s), 'stripe.balance', {
        key: cacheKey('stripe.balance', s),
        ttlSeconds: CACHE_TTL_SECONDS,
      }),
    'stripe.revenue': (s) =>
      withFallback(hasEnv(ENV), () => liveRevenue(s), () => mockRevenue(s), 'stripe.revenue', {
        key: cacheKey('stripe.revenue', s),
        ttlSeconds: CACHE_TTL_SECONDS,
      }),
    'stripe.payments': (s) =>
      withFallback(hasEnv(ENV), () => livePayments(s), () => mockPayments(s), 'stripe.payments', {
        key: cacheKey('stripe.payments', s),
        ttlSeconds: CACHE_TTL_SECONDS,
      }),
  },
};
