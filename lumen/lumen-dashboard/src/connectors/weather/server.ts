...
async function openMeteo(place: Place, extra: Record<string, string>): Promise<unknown> {
  ...
  try {
    const res = await debugFetch('weather', `${API}?${params.toString()}`, { signal: controller.signal, cache: 'no-store' });
...
const connector: ConnectorServer = {
  ...
  handlers: {
    'weather.current': (s) => {
      ...
      return withFallback(
        true,
        () => cached(cacheKeyFor('weather.current', s), CACHE_TTL_SECONDS, () => liveCurrent(place)),
    ...
    'weather.forecast': (s) => {
      ...
      return withFallback(
        true,
        () => cached(cacheKeyFor('weather.forecast', s), CACHE_TTL_SECONDS, () => liveForecast(place, days)),
